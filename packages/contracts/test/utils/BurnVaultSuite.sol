...
/// @notice The full BurnVault test suite, parameterised by a PoolManager and a USDG token so it runs
///         both against a locally deployed PoolManager and on a Robinhood Chain fork.
abstract contract BurnVaultSuite is Test {
    ...
    PoolKey internal usdgPool;
    PoolKey internal ethPool;
    PoolKey internal usdgEthPool;

    function _setUp(IPoolManager manager_, address usdg_) internal {
        manager = manager_;
        usdg = IERC20(usdg_);
        payhole = new MockERC20("PayHole", "PAYHOLE", 18);
        seeder = new LiquiditySeeder(manager);
        vault = new BurnVault(address(manager), usdg_, safe);

        usdgPool = _poolKey(usdg_, address(payhole), 3000, 60);
        ethPool = _poolKey(address(0), address(payhole), 3000, 60);
        // Deliberately unusual fee tier so the fork run never collides with a real USDG/ETH pool.
    ...
    function _fund(address token, address to, uint256 amount) internal virtual {
        ...
        else deal(token, to, IERC20(token).balanceOf(to) + amount);
    }
    ...
    function _initAndSeed(PoolKey memory k, address tokenA, uint256 amtA, uint256 amtB) internal {
        ...
        _fund(Currency.unwrap(k.currency0), address(seeder), amt0 + (k.currency0.isAddressZero() ? 1 ether : 1e6));
    ...
    function test_constructor_zeroAddressesRevert() public {
        ...
        new BurnVault(address(0), address(usdg), safe);
        vm.expectRevert(OwnerSweep.ZeroAddress.selector);
        new BurnVault(address(manager), address(0), safe);
    }
    ...
    function test_setRoute_validation() public {
        ...
        vm.expectEmit(address(vault));
        emit BurnVault.RouteSet(address(usdg), 2);
    ...
    function test_burnWith_usdgSingleHop() public {
        ...
        assertGt(burned, 0);
        // roughly 1000 PAYHOLE per USDG minus fee and impact
    ...
    function test_burnWith_eth() public {
        _configure();
        uint256 deadBefore = payhole.balanceOf(DEAD);
        vm.prank(user);
    ...
    function test_burnWith_usdgTwoHops() public {
        vm.startPrank(safe);
        vault.setToken(address(payhole));
        vault.setRoute(address(usdg), _route(usdgEthPool, ethPool));
        vm.stopPrank();
        ...
        // 300 USDG -> ~0.1 ETH -> ~300_000 PAYHOLE, minus two fees
    ...
    function testFuzz_burnWith_eth(uint96 amount) public {
    ...
    function test_burnHeld_usdgAnyoneCanFlush() public {
        _configure();
        vm.prank(user);
        assertTrue(usdg.transfer(address(vault), 50e6));
        uint256 deadBefore = payhole.balanceOf(DEAD);
        vm.expectEmit(true, true, false, false, address(vault));
        emit BurnVault.Burned(keeper, address(usdg), 50e6, 0);
        vm.prank(keeper);
    ...
    function test_burnHeld_eth() public {
        _configure();
        vm.prank(user);
    ...
    function test_burnHeld_payholeForwardedWithoutSwap() public {
        ...
        uint256 deadBefore = payhole.balanceOf(DEAD);
        vm.expectEmit(address(vault));
        emit BurnVault.Burned(keeper, address(payhole), 123e18, 123e18);
        vm.prank(keeper);
