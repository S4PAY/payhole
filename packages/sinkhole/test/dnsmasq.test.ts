...
import { dnsmasqArgs, isUpstream, renderBlocklistConfig, renderDnsmasqConfig } from "../src/render/dnsmasq.js";
...
describe("renderDnsmasqConfig", () => {
  ...
  it("renders every setting without query logging", () => {
    ...
    expect(text).not.toMatch(/log-queries|log-facility/);
...
describe("DnsmasqSupervisor", () => {
  ...
  beforeAll(async () => {
    ...
    await writeFile(fake, ["#!/bin/sh", "exec sleep 30", ""].join(NL), { mode: 0o755 });
  ...
  it("renders the files, restarts only when the blocklist changes, and stops cleanly", async () => {
    const supervisor = new DnsmasqSupervisor({
      binary: fake,
      confDir: join(dir, "conf"),
      settings: { listen: "127.0.0.1", port: 5354, upstream: ["1.1.1.1"], cacheSize: 100, user: undefined },
    });
    await supervisor.start(["a.example"]);
    ...
    expect(await supervisor.apply(["a.example"])).toBe(false);
    expect(supervisor.status().pid).toBe(pid);

    expect(await supervisor.apply(["a.example", "b.example"])).toBe(true);
    await new Promise((resolve) => setTimeout(resolve, 200));
    expect(supervisor.running).toBe(true);
